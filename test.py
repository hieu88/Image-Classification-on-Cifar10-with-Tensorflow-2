...
@tf.function
def test_step(test_aug, model, data, target):
    pred = model(test_aug(data), is_training=True)
...
def run(test_dataset, test_aug, model):
    print("Testing...")
    test_losses = []
    corrects = 0
    loader = tqdm(test_dataset)
    for data, target in loader:
        test_loss, correct = test_step(test_aug, model, data, target)
