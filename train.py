...
@tf.function
def train_step(train_aug,model,data,target,optimizer):
    with tf.GradientTape() as tape:
        pred = model(train_aug(data),is_training = True)
...
def run(train_dataset,train_aug,model,optimizer,steps_per_epoch):
    print("TRAINING...")
    train_losses = []
    corrects = 0
    loader = tqdm(train_dataset)
    for i,(data,target) in enumerate(loader):
        train_loss,correct = train_step(train_aug,model,data,target,optimizer)
        corrects += correct
